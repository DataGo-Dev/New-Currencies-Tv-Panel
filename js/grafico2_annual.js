...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var valoratual = {};
    $.getJSON('https://api.cotacoes.uol.com/currency/intraday/list/?format=JSON&fields=bidvalue,askvalue,maxbid,minbid,variationbid,variationpercentbid,openbidvalue,date&currency=5&', function(data2) {

        var dadoParsed = data2.docs;
        $("#valComEUR").text('R$ ' + dadoParsed[0].bidvalue);
        $("#valVenEUR").text('R$ ' + dadoParsed[0].askvalue);
        $("#valMaxEUR").text('R$ ' + dadoParsed[0].maxbid);
        $("#valMinEUR").text('R$ ' + dadoParsed[0].minbid);
    ...
    $.getJSON('https://api.cotacoes.uol.com/currency/interday/list/years/?format=JSON&fields=bidvalue,askvalue,maxbid,minbid,variationbid,variationpercentbid,openbidvalue,date&currency=5&', function(data) {
        ...
        for (let index = valormax - 1; index > -1; index--) {
            ...
            dadoReadyEUR.push([datapadrao, dadoParsed[index].bidvalue])
        }
        dadoReadyEUR.push([toTimestamp(toDateandTime(valoratual.date)) - 10800000, valoratual.bidvalue]);
