...
document.addEventListener('DOMContentLoaded', function() {
    $.getJSON('https://api.cotacoes.uol.com/currency/intraday/list/?format=JSON&fields=bidvalue,askvalue,maxbid,minbid,variationbid,variationpercentbid,openbidvalue,date&currency=5&', function(data) {

        var dadoParsed = data.docs;
        var valormax;
        $("#valComEUR").text('R$ ' + dadoParsed[0].bidvalue.toFixed(4));
        $("#valVenEUR").text('R$ ' + dadoParsed[0].askvalue.toFixed(4));
        $("#valMaxEUR").text('R$ ' + dadoParsed[0].maxbid.toFixed(4));
        $("#valMinEUR").text('R$ ' + dadoParsed[0].minbid.toFixed(4));
